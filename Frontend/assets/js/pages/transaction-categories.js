...
window.TransactionCategoryPage = class {
    ...
    async loadData() {
        try {
            this.transactionCategories = await vomoAPI.getTransactionCategories();
    ...
    async deleteCategory(category) {
        try {
            ...
            if (result) {
                await vomoAPI.deleteTransactionCategory(category.id);
    ...
    async saveCategory() {
        try {
            ...
            if (this.currentCategory) {
                ...
                await vomoAPI.updateTransactionCategory(this.currentCategory.id, formData);
                DevExpress.ui.notify('Category updated successfully', 'success', 3000);
            } else {
                // Create new category
                await vomoAPI.createTransactionCategory(formData);
