...
window.ZonePage = class {
    ...
    async saveOffices() {
        ...
        try {
            ...
            const updatedZone = await vomoAPI.updateZone(this.currentZone.id, {
    ...
    async handleRowInserting(e) {
        try {
            const result = await vomoAPI.createZone(e.data);
            e.data.id = result.id;
    ...
    async handleRowUpdating(e) {
        try {
            await vomoAPI.updateZone(e.key.id, {...e.oldData, ...e.newData});
    ...
    async handleRowRemoving(e) {
        try {
            await vomoAPI.deleteZone(e.key.id);
