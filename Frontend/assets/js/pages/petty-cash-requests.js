...
window.PettyCashRequestsPage = class {
    ...
    async loadRequestForEdit(requestId) {
        try {
            const request = await vomoAPI.getPettyCashRequest(requestId);
    ...
    async saveRequest() {
        try {
            ...
            if (this.currentRequest) {
                await vomoAPI.updatePettyCashRequest(this.currentRequest.id, formData);
                DevExpress.ui.notify('Request updated successfully', 'success', 3000);
            } else {
                await vomoAPI.createPettyCashRequest(formData);
    ...
    async deleteRequest(request) {
        ...
        if (result) {
            try {
                await vomoAPI.deletePettyCashRequest(request.id);
