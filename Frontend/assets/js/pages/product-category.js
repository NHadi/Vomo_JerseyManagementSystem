...
window.ProductCategoryPage = class {
    ...
    async loadData() {
        try {
            ...
            const data = await vomoAPI.getProductCategories();
    ...
    async handleRowInserting(e) {
        try {
            const result = await vomoAPI.createProductCategory(e.data);
            e.data.id = result.id;
    ...
    async handleRowUpdating(e) {
        try {
            await vomoAPI.updateProductCategory(e.key.id, {...e.oldData, ...e.newData});
    ...
    async handleRowRemoving(e) {
        try {
            await vomoAPI.deleteProductCategory(e.key.id);
