...
window.PurchaseOrderPage = class {
    ...
    async loadData() {
        try {
            this.purchaseOrders = await vomoAPI.getPurchaseOrders();
    ...
    async loadSuppliers() {
        try {
            this.suppliers = await vomoAPI.getSuppliers();
    ...
    async loadItems() {
        try {
            this.items = await vomoAPI.getItems();
    ...
    async saveOrder() {
        try {
            ...
            if (this.currentOrder) {
                ...
                await vomoAPI.updatePurchaseOrder(this.currentOrder.id, formData);
                DevExpress.ui.notify('Order updated successfully', 'success', 3000);
            } else {
                // Create new order
                await vomoAPI.createPurchaseOrder(formData);
